...
This concept is illustrated in Figure \ref{fig:forward_APDDM}. The forward diffusion path (indicated in orange) gradually moves from the high-SNR image $x_0$ toward a distribution centered around the noise image $x_T^*$, with each step adding increasing amount of noise. 
...
\begin{equation}\label{eq:Markov chain prediction}
...
Figure \ref{fig:full_APDDM} shows the training strategy. The clean image $x_0$ is transformed into $x_t$ via forward diffusion and the network is trained to reverse this transformation by removing predicted noise and minimizing the loss. 
...
After hyperparameter tuning, the model was ran again. Hereby, the parameters of section 7.2 were used.
...
\subsection{APDDM Results}
A simplified version of the APDDM forward process was implemented to train a new UNet model. As a simplification, only images from the last timestep were fed to the network. This way there was no need for a time-embedding. This approach functioned as a type of data augmentation.
Some examples of test mice slices from the simplified APDDM model, compared to the same slices from the base UNet model can be seen in Figure \ref{fig:HT_coronal_Diffusion}\\
More examples of each combination of region and anatomical Plane can be found in Appendix \ref{Appendix: APDDM Results}

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Coronal_14_Diffusion.png}
    \caption{Simplified APDDM model output of a test mouse slice. }
    \label{fig:HT_coronal_Diffusion}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Coronal_14_UNet.png}
    \caption{Base UNet model output of a test mouse slice. }
    \label{fig:HT_coronal_UNet}
\end{figure*}
...
\newpage
\begin{appendices}
\section{Appendix: Work progression (no deviation from the orginal planning was made)}
