...
Once bad slices were identified, they were removed from the used dataset (see appendix \ref{Appendix: Figures Preprocessing}: Figure~\ref{fig:bad_slice}). 6.7\% of the slices were removed. 
%vervang percentage verwijderde slices naar aantal slices verwijderd. 
...
%the subsection below is data augmentation and not pre processing
...
In this project, at least 10 epochs will be run. If there is no improvement during 6 epochs, the program will be stopped.
...
In this project, four different optimizers will be tested: Adam (short for adaptive moment estimation), AdamW, SGD and RMSprop. 
%briefly explain how this works ie. a moving average of past gradients is used to update parameters. This means it doesn't rely solely on the current gradient but also "remembers" previous ones. The moving average helps smooth out noisy updates and provides a more stable direction — similar to momentum — which helps the optimizer converge faster and more reliably, especially in noisy or sparse data settings.
...
Adam uses a fixed learning rate, while stochastic gradient descent (SGD) updates the model's parameteres based on the gradient loss function  \cite{massed-compute-2025}. 
...
The scheduler adjust the learning rate during training according to the predefined strategy. 
%look up real names
The following schedulers were be tested: exponential learning rate [\texttt{ExponentialLR}], step learning rate [\texttt{StepLR}], and reduce learning rate on plateau [\texttt{ReduceLROnPlateau}].
...
Their respective formulas [4] and [5] are given below.
...
This algorithm decreases the learning rate when the specified metrics stops improving for longer than allowed. 
...
The learning rate decay decreases the learning rate when the training progresses and becomes more stable. 
...
%rewrite next part in simpler terms
...
%look up real names
Several methods are available, including nearest, linear, bilinear, trilinear, and bicubic interpolation, as implemented in torch.nn.Upsample.
...
This method is fast and simple, but might lead to produce pixelated images.
...
A learning rate value which is too high can lead to instability and poor convergence, while too low value might slow down the training of the model  as can be seen on Figure \ref{fig:Learnrate} or it can even get stuck in a local minima \cite{jordan_2018_setting}. 
...
The number of epochs defines how many times the model goes through the entire training dataset for training. 
...
Since MSE does not account for structural composition, structural similarity index measure (SSIM) is used. 
...
\begin{figure}
    ...
    \caption{Visual representation of the loss function over different epochs during hyperparameter training. geen assen!!!}
...
\begin{figure*}
    ...
    \caption{First results of abdomen}
...
\begin{figure*}
    ...
    \caption{First results of  transaxial plane after 11th epoch.}
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{loss_graph.png}
...
Unlike other methods, upconv layers can learn to reintroduce high-frequency detail, making them well suited in this case to enhance the resolution. 
...
Because of formula \ref{eq:6} it can be concluded that structural similarity is favored, encouraging the model to preserve structural features over strict pixel accuracy. 
...
%add reason why validation relied on MSE
