...
For each mouse, scans of multiple regions and along different planes were acquired.
...
A feasible schedule was proposed and subsequently followed. \\
...
Also, we would like to express our gratitude to the MEDISIP team for providing the opportunity to use the INFINITY lab. 
...
Dedicated micro-MRI systems are available for preclinical imaging that provides excellent soft tissue contrast for anatomical visualization in rodent models \cite{marzola2003high}. 
To visualize the animal's anatomy with sufficient detail, a high spatial resolution is important. Reducing voxel size improves spatial resolution but also decreases the signal and lowers the signal-to-noise ratio (SNR). 
On the other hand, increasing acquisition time can enhance signal quality and SNR \cite{grover2015mri}. However, this increases the risk of artifacts, blurring, and motion-related distortions.
...
algorithms. More recently, deep learning (DL) has emerged, which can learn to restore fine details and reduce noise directly from
examples of high and low quality images which has been demonstrated to provide better results. These models address the trade-off between resolution and image quality \cite{brown2014magnetic}. 
...
A Bruker pharmascan was used to acquire images for the DL network. To avoid contamination of the lab, the operator must wear a hairnet, face mask, lab coat, latex gloves and shoe covers. 
...
A dataset of 19 mice was already acquired and provided to us. These made use of the rat brain surface coil. It this project the same coil was used to obtain additional images to maintain uniformity in data acquisition.
...
All scans were acquired using a T2-weighted rapid acquisition with refocused echoes (RARE) sequence. 
...
As a result, fluids will appear brighter than muscle and fat in the image. 
...
Tissues with longer T2 values maintain more signal and will look brighter. TR refers to the time between successive excitations of the same tissue slice. 
...
The matrix size determines the level of detail in an image with the high-resolution (HR) scan (300x300 pixels) capturing finer structures compared to that of the low-resolution (LR) scan (150x150 pixels). 
...
To significantly enhance SNR by reducing noise, the HR scan uses five averages per slice.\\
\\
...
\begin{table}[h]
\centering
\caption{\label{tab:mri} Overview of the scan and image parameters used to acquire high- and LR data of mice.}
...
Pre-processing of the slices is a crucial step in preparing the data for the DL model. 
...
To counter this problem the images were cropped ensuring that the region of interest was intact (see appendix \ref{Appendix: Figures Preprocessing}: Figure~\ref{fig:comparision-cropped}).  
...
The LR image were upsampled using nearest-neighbor interpolation (order=0) with a zoom factor of 2 to match the HR image dimensions. 

\subsection{Normalization}

Mean normalization was performed, each LR image was divided by its mean intensity, and the same mean value was applied as a normalization factor for the corresponding HR image.  
Both LR and HR images were converted for compatibility with PyTorch models.
...
This is done during network training and improves the model generalization. Since input may have varying dimensions, the images were padded to match the largest width and height within each batch, ensuring uniform sizes for batch processing.


\section{Neural Network and Training Process}
\subsection{Encoder-Decoder Denoising Model}
For the denoising DL model, a convolutional neural network based on the U-Net architecture was used. 
...
The input to the network is a LR ($150 \times 150$ pixels), which is first upsampled via interpolation to match the HR target size ($300 \times 300$ pixels). 
The network then processes this input to produce a denoised output with a higher signal-to-noise ratio (SNR).

The encoder comprises of a series of convolutional layers with activation functions followed by downsampling steps (e.g., pooling or strided convolutions). 
These layers extract hierarchical features while reducing spatial resolution and increasing the number of feature channels \cite{tran2025unet}.
...
To get the best results for the DL model for micro MRI image denoising and super-resolution, the hyperparameters need to be fine-tuned. 
Given the number of components and interactions within the model, manual tuning would be inefficient and inconsistent. 
Therefore, the open-source optimization software Optuna was used \cite{datacamp_optuna}. 
...
This combination is especially helpful to reduce the number of trainings that need to be run, and to save computational resources. 
...
Overall Optuna played a key role in helping reach a well-performing model more efficiently, and ensured that the final configuration was based on data-driven search rather than trial and error.
...
LeakyReLU addresses this problem by allowing small negative outputs \cite{relu}. This function \ref{eq:5} helps keep neurons active and keeping them active. 
...
With residual learning instead of predicting the final HR images the model learns the difference between the low- and HR, this is called the residual. 
...
One of Adam's limitations is the way it incorporates L2 regularization directly into the gradient update, which can sometimes negatively affect the performance. 
Stochastic gradient descent (SGD) updates the model’s parameters based on the gradient loss function. Unlike Adam, it uses a fixed learning rate and does not estimate the moments of the gradients \cite{massed-compute-2025}.
...
The loss function defines how well the model is performing by evaluating the prediction against the target images. 
It guides the process by providing a measure of error which then can be minimized during training. 
Three loss functions were tested: Mean Squared Error (MSE) , Mean Absolute Error (MAE) and a hybrid loss. 
...
A schematic representation can be found in Figure \ref{fig:PDDM} and Figure \ref{fig:APDDM} for DDPM and APDDM, respectively.
Unlike DDPM, which is trained on unpaired data and gradually adds Gaussian noise until the image becomes entirely random, APDDM is adapted for use with paired high- and low-SNR MRI images, in this case, the 12-minute HR scans and the 1-minute LR scans.
...
Figure \ref{fig:APDDM_train} shows the training strategy. The clean image $x_0$ is transformed into $x_t$ via forward diffusion and the network is trained to reverse this transformation by removing predicted noise and minimizing the loss. 
This setup allows the network to focus on denoising realistic degraded images, guided by noise from the data.

\begin{figure*}
    \centering
    \includegraphics[width=0.8\linewidth]{full APDDM .png}
    \caption{Training overview of APDDM.}
    \label{fig:APDDM_train}
\end{figure*}

\subsubsection{Integration}
APDDM is particularly well suited for the micro-MRI dataset that is used, where paired scans with varying SNR levels are available. 
By using real residuals between the paired images as noise sample, the forward process creates realistic, data-driven noisy versions of the HR images. 
...
In the script, the training, validation and test mice were defined. A detailed overview can be found in Table \ref{tab:mouse_distribution}.
...
The visual comparision will compare the residuals of HR and LR images to the residuals of HR and DL images. 
Ideally the residual of HR and images should be close to zero in most regions, indicating an accurate reconstruction. 
...
The MSE was calculated for the high resolution and the result of the DL model. After these calculations the ratio is taken of these two.
...
In this context, SSIM is used to compare the predicted images to the HR reference scans.  
...
Next to the CNR, the normalised standard deviation was also calculated. This follows the same procedure as the CNR and is defined as:
\begin{equation}\label{eq:normsigma}
\sigma_{\text{norm}} = \frac{\sigma_b}{\mu_b}
\end{equation}
...
The training was done on the accelgor cluster because of its GPU capability. 
...
\begin{figure}
    ...
    \caption{Visual representation of the loss function over different epochs.}
...
After hyperparameter tuning, the model was run again. Hereby, the parameters of section 7.2 were used.
...
\subsection{APDDM}
A simplified version of the APDDM forward process was implemented to train a new UNet model. 
As a simplification, only images from the last timestep were fed to the network. 
This way, there was no need for a time-embedding. 
This approach functioned as a type of data augmentation. 
Some examples of test mice slices from the simplified APDDM model, compared to the same slices from the base UNet model can be seen in Figure \ref{fig:diffusion_example} and \ref{fig:unet_diffusion_comparison}. More examples of each combination of region
and anatomical plane can be found in Appendix \ref{Appendix: APDDM Results}.

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Coronal_14_Diffusion.png}
    \caption{Simplified APDDM model output of a coronal, head-thorax test mouse slice. }
    \label{fig:diffusion_example}
\end{figure*}

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Coronal_14_UNet.png}
    \caption{Base UNet model output of a coronal, head-thorax test mouse slice. }
    \label{fig:unet_diffusion_comparison}
\end{figure*}
...
The DL output is somewhat sharper: for example, the white spinal column and rib edges within the yellow-highlighted region are brighter and better defined than in LR. 
...
In Figure \ref{fig:val_2} the yellow arrow is pointing to the lung cavity, where the DL output preserves some fine internal structures,
...
\begin{table*}[h!]
    \centering
    \caption{\label{tab:MSE_SSIM} MSE and SSIM values. TA stands for thorax-abdomen, while HT is used for head-thorax.}
...
The final values for the CNR are represented in Table \ref{tab:CNR} and the normalised standard deviation values are shown in Table \ref{tab:norm_var}.
...
\begin{sidewaystable*}[h!]
    \centering
    \caption{\label{tab:norm_var} Overview of the mice, slices and planes used to obtain normalised standard deviation in LR, DL and HR images. TA stands for thorax-abdomen, while HT is used for head-thorax.}
    \begin{tabular}{l|c|c|c|c|c|c|c}
    \toprule
     & \textbf{Mouse} & \textbf{Plane} & \textbf{Slice} & \textbf{Region} & \textbf{LR normalised standard deviation} & \textbf{DL normalised standard deviation} & \textbf{HR normalised standard deviation} \\
    \midrule
...
Figure \ref{fig:first_sagittal}, Figure \ref{fig:first_transax} and Figure \ref{fig:first_coronal} shows that the DL image has clearly higher quality and sharper details compared to the low resolution (LR) image at the left side. 
...
Several key hyperparameter choices reflect current best practices in DL for medical image restoration, while others diverge in notable ways

The feature dimensions were [128,128,64], this does deviate from the typical dimension used in a U-Net design where feature depth increases with depth (e.g. [64,128,256]).
...
It is also a possibility that the somewhat unintuitive result is by random chance, as the search space of the hyperparameters is very large. Optuna must sparsely search this space, so some hyperparameters might not be completely optimal. 

Strided convolution (convStrided) was preferred over pooling for downsampling. 
This aligns with expectations from common CNN practice. 
Strided convolution allows the model to practice how to compress feature maps instead of relying on fixed operations such as meanpooling. 
...
Unlike transposed convolutions, which introduce additional parameters and are prone to artifacts, bilinear upsampling provides a smooth and stable to restore spatial dimensions. 
...
In this case the LR and HR images were relatively similar in structure and intensity, making transformations less complex.
...
This balance is preferred for restoring subtle textures in HR data. 
...
The background also needs to be taken into account, which can also lead to other maximum intensities.
...
\subsection{APDDM}
First of all, it must be noted that the APDDM model is trained on slightly different normalisation techniques and hyperparameters, in the intrest of time. 
It can be observed that the background noise around the mouse gets smoothed out nicely, while still preserving and sharpening the edges of the anatomy. \\
When comparing with the results of the base UNet, the darkening of certain regions of the images is less of a problem in the case of the APDDM results, which might be attributed to the use of a different normalization technique. 
Some parts of the anatomy do get oversmoothed like in the case of the base UNet, as discussed above, which results in some anatomical structures not being visible, while being visible on the HR images. \\
The most import part is that the simplfied APDDM was succesfully implemented, and was able to denoise the images, as seen by comparing HR-LR and HR-DL images.
...
This is due to the network learning to average high-frequency patterns which results in blurred textures. 
...
MSE encourages finding pixel-wise averages of solutions, producing overly-smooth results. A visual inspection confirms this.

\subsubsection{MSE and SSIM}
High SSIM values in Mouse 23 (Transax slice 4) demonstrate accurate preservation of simpler structures. The lower SSIM in Mouse 7 (Sagittal slice 11) however, suggests difficulty in accurately reconstructing its more complex anatomical details. The overall low MSE values, imply that the overall pixel intensity is preserved well in the DL reconstruction.  
...
After these observations were made, the normalized standard deviation was also evaluated. As shown in Table \ref{tab:norm_var}, a clear and consistent pattern emerges: the DL-normalized standard deviation is consistently the lowest across all cases, while the LR-normalized standard deviation is consistently the highest.
This trend can be attributed to the behavior of the DL model, which is trained to reduce the noise while preserving structural information. As a result, the standard deviation of the background region is significantly reduced, while the mean background intensity remains relatively stable.
This leads to lower normalized standard deviation values for DL images, which is indicative of improved image quality and less noise.
Conversely, the LR image exhibit higher normalized standard deviation due to increased noise levels. In these images, the mean of the background is generally higher.
...
\end{figure*}
As an extra validation step, a MRI scan of a LEGO brick with a 1-minute scan time was given as input to the trained model and compared with its 12-minute scan time counterpart. 
This can show whether the model is overtrained and whether it could potentially be used to denoise and improve shorter scan time images of objects other than mice. 
This also provides a phantom-object of known shape and dimensions, with sharp edges, which can be used to assess the super-resolution performance in better retaining sharper borders.
The images discussed here can be seen in Figure \ref{fig:lego_phantom_test}.
...
The MSE between the DL and HR images is 0.1269, and between the LR and HR images is 0.0737 (an increase of 72\% from the LR to the DL image). This means that if only the pixel-wise error is considered, the LR image remains a better approximation of the HR image than the DL image.
...
The project successfully developed and validated 2 DL frameworks, U-Net and APDDM, to enhance micro-MRI images through denoising. 
Both approaches achieved improvements in image quality, allowing for reduced acquisition times, while delivering image quality that improves anatomical visibility compared to LR input, though still not fully equivalent to HR scans. 

The U-net model, after hyperparameter tuning via an optimized search framework with Optuna, demonstrated noise reduction. 
However, due to the loss function MSE, the model showed a tendency to oversmooth fine anatomical details. 
This highlights a limitation of pixel-wise loss function in not preserving detailed textures and contrast, which is especially valuable in medical imaging.
...
Validation through both quantitative metrics and visual inspection confirmed that the model enhances LR scans, potentially enabling 1-min scans in preclinical MRI settings, but further model optimization and more training data is required to result in more consistent and robust outcomes.
...
In summary, this project forms a foundation for integrating DL into preclinical imaging workflows, aiding in faster acquisition times without sacrificing diagnostic value. 
Future improvements could explore alternative loss functions, such as SSIM or perceptual losses, and expand the training dataset to include more variation. 
...
\begin{thebibliography}{99}
    ...
    \bibitem{marzola2003high} P. Marzola, F. Osculati, and A. Sbarbati (2003). “High field mri in preclinical research”  \textit{European Journal of Radiology}, 48(2), 165–170.
...
\\
First a procedure is defined, where a linear interpolation is done between $\bold{x}_0$ and $\bold{x}_T^*$, and add a not yet specified noise term:
\begin{equation}
    x_t = (1-\alpha_t)\cdot\bold{x}_0 + \alpha_t\cdot\bold{x}_T^* + noise(t)
\end{equation}

\subsection{Proposition of the Markovian process}
Now this is transformed into a Markovian process, by allowing the endpoint to not be exactly $\bold{x}_T$, but rather let the endpoint $\bold{x}_T$ be $\bold{x}_T^*$ in expected value.\\
\\
If $\bold{x}_t$ is subtracted by $\bold{x}_{t-1}$, which gives:
\begin{equation}
...
\end{equation}
By using a Markov process, a natural diffusion path is created from $\bold{x}_0$ to $\bold{x}_t$, where each step builds up on the previous one. 
Now define the final Markov process is defined as: 
...
Where $f(\alpha_t)$ is defined to be a function that is zero for arguments $0$ and $1$, and reaches a maximum of 1 somewhere in the range $[0,1]$. This is the noise schedule function. Let:
\begin{equation}
...
\\
The noise term $\epsilon$ should be distributed according to the real noise in the data:
\begin{equation}
    \epsilon \sim P[noise]
\end{equation}
Approximate this empirically by sampling from all the observed residuals in the dataset:
\begin{equation}
...
\\
Apply a Markov chain T times to the input $\bold{x}_0$:\\
\begin{equation}
...
\end{equation}
If assuming $\mathbb{E}[\hat{\epsilon}] = 0$:
\begin{equation}
    \boxed{\mathbb{E}[\bold{x}_T] = \bold{x}_T^*}
\end{equation}

\subsection{Variance of the endpoint}\label{appendix:var}
Now derive the expression for the variance on the enpoint $\bold{x}_T$:
\begin{multline}
...
\end{multline}
Where $Cov[\hat{\epsilon}_i, \hat{\epsilon}_j] = 0$ is used because the noise samples are independent. \\
Now using the notation: $Var[\hat{\epsilon}_t] = \sigma_{\epsilon}^2$, and substituting (\ref{eq:f(t)}), which gives:
\begin{equation}
...
\\
Also the general variance throughout the whole path (not just at the endpoint) can be derived:
\begin{equation}
...
\\
It can observed that the variance increases slowly in the beginning, and before the endpoint, it slows down and stabilizes again. This is the reason for the particular choice of the noise schedule function $f(t)$ discussed previously. \textbf{However, a lot of noise schedule choices can be made and should be investigated.}

\subsection{Controlling the variance}
I will now propose a method of controlling the variance of this path. This way, the variance of the endpoint as a variable parameter can be obtained, and even optimize the model in function of this parameter. Thus the ``degree of noise'' can be controlled.   
First, the sampled residual $\hat{\epsilon}$ is normalized, so given the variance of the noise is 1:
\begin{equation}
    \hat{\epsilon'} = \hat{\epsilon} / s_{n,\epsilon}
\end{equation}
($s_{n,\epsilon}$ = population variance of $\mathcal{E}_{dataset}$)\\
Additional note: an empirical noise distribution has an expected value of 0.\\
Then the variance is controlled by adding a parameter $\sigma$:
\begin{equation}
...
\end{equation}
Now, the variance of the endpoint is: 
\begin{equation}
    Var[\bold{x}_T] = \beta^2
\end{equation}
Now there is full control over the variance of the endpoint, with exception to the stability criterion as described in the following section. 

\subsection{MSE stability}
Important to note is that there are some constraints on this parameter beta (and thus the variance of the noise term), if the diffusion path needs to converge to $\bold{X}_T^*$. Measuring this in the form of the MSE between $\bold{x}_T$ and $\bold{x}_T^*$, which should not be exceedingly large. \\
An important condition that must be met, is that $\bold{x}_t$ becomes more similar to $\bold{x}_T^*$ each step.\\
\\
 This condition will not be met if the noise term overpowers the term $\frac{1}{T}  \bold{R}$ at a certain step. In other words, the residual term magnitude should be larger than the noise term magnitude in each step, so that the diffusion path still mostly converges towards $x_T^*$. An estimation of the size of the subsequent terms will be made based on their Mean Square value $\frac{1}{i,j}\sum_{i,j} X_{i,j}^2$ (MS value). Also, let $f(t) = 1$, to have a conservative estimation.  \\
...
Substituting (\ref{eq:beta}) gives the criterion in terms of $\beta$:
\begin{equation}
    \beta \lesssim \sqrt{\frac{8}{15} \frac{T^4-1}{T^5}} RMS(\bold{R}) = \beta_{crit}
\end{equation}
\\
This result can be demonstrated by a simulation where beta is varied and the mean square error between $x_T$ and $x_T^*$ is computed over N diffusion path samples (Fig. \ref{fig:beta sweep}). It can be indeed observed that if $\beta$ is within the order of magnitude of $\beta_{crit}$, the diffusion process won't converge towards $x_T^*$ anymore, which manifests itself in an increase in the MSE. 
\begin{figure}[h!]
    \centering
...
\FloatBarrier
\newpage
\subsection{Appendix: APDDM Results}
\label{Appendix: APDDM Results}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Sagittal_14_Diffusion.png}
    \caption{Simplified APDDM model output of a sagittal, head-thorax test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Sagittal_14_UNet.png}
    \caption{Base UNet model output of a sagittal, head-thorax test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Transax_20_Diffusion.png}
    \caption{Simplified APDDM model output of a transaxial, head-thorax test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_HEAD-THORAX_Transax_20_UNet.png}
    \caption{Base UNet model output of a transaxial, head-thorax test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_THORAX-ABDOMEN_Coronal_17_Diffusion.png}
    \caption{Simplified APDDM model output of a coronal, thorax-abdomen test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_THORAX-ABDOMEN_Coronal_17_UNet.png}
    \caption{Base UNet model output of a coronal, thorax-abdomen test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_THORAX-ABDOMEN_Sagittal_17_Diffusion.png}
    \caption{Simplified APDDM model output of a sagittal, thorax-abdomen test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse06_THORAX-ABDOMEN_Sagittal_17_UNet.png}
    \caption{Base UNet model output of a sagittal, thorax-abdomen test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse10_THORAX-ABDOMEN_Transax_16_Diffusion.png}
    \caption{Simplified APDDM model output of a transaxial, thorax-abdomen test mouse slice. }
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{Mouse10_THORAX-ABDOMEN_Transax_16_UNet.png}
    \caption{Base UNet model output of a transaxial, thorax-abdomen test mouse slice. }
\end{figure}
