...
This study presents a deep learning (DL) framework designed to enhance image quality and reduce acquisition time by combining denoising and super-resolution techniques. 
Paired T2-weighted scans were acquired for 24 mice using a 7T Bruker Pharmascan system, consisting of low-resolution (LR)(1-minute) and high-resolution (HR)(12-minute) images.
...
They also give their input to develop the DL together with Daan and Darío.
...
algorithms. More recently, DL has emerged, which can learn to restore fine details and reduce noise directly from
...
A Bruker pharmascan was used to acquire images for our DL network. To avoid contamination of the lab, the operator must wear a hairnet, face mask, lab coat, latex gloves and shoe covers. 
...
A dataset of 19 mice was already acquired and provided to us. These made use of the rat brain surface coil. The same coil was used to obtain additional images to maintain uniformity in data acquisition
...
The matrix size determines the level of detail in an image with the high-resolution scan (300x300 pixels) capturing finer structures compared to that of the LR scan (150x150 pixels). 
...
In the project, the data of 5 mice was acquired.
...
The input to the network is a LR image ($150 \times 150$ pixels), which is first upsampled via interpolation to match the Hr target size ($300 \times 300$ pixels). 
...
A built-in sampler selects new parameter values to explore, while a pruner stops trials early if the model is clearly not improving. 
...
When the stride is greater than 1, the convolution operation skips some of the pixels, which results in an effective method to reduce the size of the feature map \cite{unknown-author-no-date2}.
...
Different options were tested such as ReLU,  LeakyReLU, ELU and PReLu. Each of these options comes with their own set of advantages and difficulties. 
...
LeakyReLU addresses this problem by allowing small negative outputs \cite{relu}. This function \ref{eq:5} helps keep neurons active. 
This technique keeps the simplicity of ReLU while improving stability. 
...
The last option, the hybrid loss, is a custom-defined loss which combines MSE and structural similarity index (SSIM).
...
MSE helps preserving pixel-wise fidelity while SSIM ensures structural integrity of the image, which is crucial in medical contexts. 
...
\begin{figure*}
    ...
    \caption{Schematic representation of the APDDM model.}
    \label{fig:APDDM}
\end{figure*}
...
The predicted noise is then subtracted to obtain an estimate of $\hat{x_0}$, and training is performed using MSE loss: 
\begin{equation}\label{eq:Markov chain}
...
The visual comparision will compare the residuals of HR and LR images to the residuals of HR and deep-learning images. 
Ideally the residual of HR and DL images should be close to zero in most regions, indicating an accurate reconstruction. 


\subsection{Mean squared error}
The MSE measures the amount of error in statistical models by taking the average squared difference between the observed and predicted values. 
...
These slices were saved as raw data files, with their shape corresponding shape information. Both were loaded into Amide. A ROI was selected and a box was drawn in it. The same procedure was followed for the background region.
Figure \ref{fig:CNR} visualises the procedure described above. 
Next to the CNR, the  standard deviation was also calculated. This follows the same procedure as the CNR and is defined as:
...
The hyperparameters were chosen based on knowledge and guessing what the best one would be. They were chosen as follows:
\begin{itemize}
...
\end{itemize}
The model was trained with a batch size of 32 and training was conducted over about 15 epochs. MSE was employed as the loss function and ReLU was the activation function.

The first epoch was the best, as can be observed in Figure \ref{fig:first_loss}.
...
Since the output did not all images back, Figure \ref{fig:first_coronal} is a result of the 13th epoch.
...
\begin{figure}
    ...
    \caption{Visual representation of the loss function of the first epoch.}
...
The same division of mice was used, as indicated in Table \ref{tab:mouse_distribution}.
...
\subsection{APDDM}
A simplified version of the APDDM forward process was implemented to train a new U-Net model. 
...
Some examples of test mice slices from the simplified APDDM model, compared to the same slices from the base U-Net model can be seen in Figure \ref{fig:diffusion_example} and \ref{fig:unet_diffusion_comparison}. More examples of each combination of region
...
\begin{figure*}
    ...
    \caption{Base U-Net model output of a coronal, head-thorax test mouse slice. }
...
\begin{figure*}
    ...
    \caption{Sagittal view of a training mouse thorax-abdomen region.}
    \label{fig:val_1}
\end{figure*}
...
\begin{figure*}
    ...
    \caption{Sagittal view of a validation mouse thorax-abdomen region.}
    \label{fig:val_2}
\end{figure*}
...
\begin{figure*}
    ...
    \caption{Sagittal view of the spine and pelvic region of a validation mouse.}
    \label{fig:val_3}
\end{figure*}
...
The obtained values of Amide were inserted in Equation \ref{eq:CNR} and Equation \ref{eq:normsigma}.
The final values for the CNR are represented in Table \ref{tab:CNR} and the standard deviation values are shown in Table \ref{tab:norm_var}.
...
\begin{sidewaystable*}[h!]
    \centering
    \caption{\label{tab:norm_var} Overview of the mice, slices and planes used to obtain standard deviation in LR, DL and HR images. TA stands for thorax-abdomen, while HT is used for head-thorax.}
    \begin{tabular}{l|c|c|c|c|c|c|c}
    \toprule
     & \textbf{Mouse} & \textbf{Plane} & \textbf{Slice} & \textbf{Region} & \textbf{LR standard deviation} & \textbf{DL standard deviation} & \textbf{HR standard deviation} \\
    \midrule
...
Unlike transposed convolutions, which introduce additional parameters and are prone to artifacts, bilinear upsampling provides a smooth and stable way to restore spatial dimensions. 
...
This allows the network to learn quickly during early epochs and refine weights more delicately later in training. 
...
The loss function was fixed as MSE, a standard objective for pixel-wise regression tasks. 
...
\subsection{Results after hyperparameter training}
When looking at the results after hyperparameter tuning, it can be seen that the model is still working quite good. 
...
The maximum intensity of the DL image is again lower than the ones of the LR and HR image, but the difference is not a lot. 
...
The most import part is that simplfied APDDM was succesfully implemented, and was able to denoise the images, as seen by comparing HR-LR and HR-DL images.
...
Furthermore, interpolation is done, which results in a smoothing. This smoothing alters the intensity distribution and direclty affects CNR.
...
The MSE between the DL and HR images is 0.1269, and between the LR and HR images is 0.0737 (an increase of 72\% from the LR to the DL image). This means that if only consider the pixel-wise error is considered, the LR image remains a better approximation of the HR image than the DL image.
...
Additionally, an external LEGO phantom scan suggested the networks’s generalization capacity, though it also exposed limitations such as smoothing due to the chosen loss function. 
...
\\
First a procedure is defined, where linearl interpolation is done between $\bold{x}_0$ and $\bold{x}_T^*$, and add a not yet specified noise term:
\begin{equation}
    x_t = (1-\alpha_t)\cdot\bold{x}_0 + \alpha_t\cdot\bold{x}_T^* + noise(t)
\end{equation}

\subsection{Proposition of the Markovian process}
Now, transforming this into a Markovian process, by allowing the endpoint to not be exactly $\bold{x}_T$, but rather let the endpoint $\bold{x}_T$ be $\bold{x}_T^*$ in expected value.\\
\\
If $\bold{x}_t$ is substracted by $\bold{x}_{t-1}$, this gets:
\begin{equation}
...
\begin{figure}[h!]
    ...
    \caption{Demonstration of coherent edge artifact accumulation when no rotation is applied. }
...
\begin{figure}[htbp]
    ...
    \caption{Base U-Net model output of a sagittal, head-thorax test mouse slice. }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Base U-Net model output of a transaxial, head-thorax test mouse slice. }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Base U-Net model output of a coronal, thorax-abdomen test mouse slice. }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Base U-Net model output of a sagittal, thorax-abdomen test mouse slice. }
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Base U-Net model output of a transaxial, thorax-abdomen test mouse slice. }
\end{figure}
...
\begin{sidewaystable}[h]
    \centering
    \caption{Overview of the mice, slices and planes used to obtain CNR and standard variation in LR, DL and HR images. TA stands for thorax-abdomen, while HT is used for head-thorax.}
    \resizebox{0.8\linewidth}{!}{
    \begin{tabular}{l|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c|c}
    \toprule
     & \textbf{Mouse} & \textbf{Plane} & \textbf{Slice} & \textbf{Region} &&\textbf{LR mean ROI} &\textbf{LR mean background} &\textbf{LR STD background}&\textbf{LR CNR}& \textbf{LR variance} & &\textbf{DL mean ROI} &\textbf{DL mean background} &\textbf{DL STD background}&\textbf{DL CNR}&\textbf{DL variance} &&\textbf{HR mean ROI} &\textbf{HR mean background} &\textbf{HR STD background}&\textbf{HR CNR}& \textbf{HR variance} \\
    \midrule
...
\section{Appendix: AI disclaimer}\label{Appendix:AI}
In this project, AI was used to assist in writing code for Optuna. This was due to the fact that, initially, it was not entirely clear how Optuna works.

By using both internet resources and ChatGPT, we gained insights into the functioning of the framework.

Additionally, when certain LaTeX commands were unclear and online sources did not provide answers, ChatGPT was consulted for support.
