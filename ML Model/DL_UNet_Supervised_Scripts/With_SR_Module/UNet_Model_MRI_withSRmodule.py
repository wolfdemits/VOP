...
class SuperResolution_Module(nn.Module):
    """ Niels Verhelst's implementation of the SR Module: 
        * Residual connection is added to the output layer -> Num of feature maps = 1
        * Apply convolutional layer to increase the number of feature maps to num_channels_SRmodule (user-defined)
        * Apply convolutional transpose layer to increase the resolution (incr. spatial dimension)
        * Apply 1x1 convolutional layer to reduce the number of feature maps to 1
        """
    
    def __init__(self, dim, num_channels_SRmodule, conv_kernel, activation):
        ...
        #   ConvBlock(dim, in_channel, out_channel, conv_kernel, activation)
        self.ConvLayer_forSR = ConvBlock(dim, 1, num_channels_SRmodule, conv_kernel, activation)

        # To increase the spatial dimensions:
        #   UpConv(dim, num_main_channel, num_skip_channel, num_channel_out, conv_kernel, activation) 
        self.UpConv_forSR = UpConv(dim, num_channels_SRmodule, 0, num_channels_SRmodule, conv_kernel, activation)

        # Output layer to reduce the spatial dimensions
        #   Conv_1x1(dim, in_channel, out_channel)
        self.OutLayer_forSR = Conv_1x1(dim, num_channels_SRmodule, 1)
        
    def forward(self, x):

        x = self.ConvLayer_forSR(x)

        x = self.UpConv_forSR(x, x, 1)  # UpConv requires x_encode and padValue as extra arguments
                                            # but only needed if there is a skip connection, which is 
                                            # not the case for the SR-Module
        x = self.OutLayer_forSR(x)
...
class UNet_withSR(nn.Module):

    def __init__(self, dim, num_in_channels, features_main, features_skip, conv_kernel_size, 
            down_mode, up_mode, activation, residual_connection, num_channels_SRmodule, PadValue=-1):
        """
        ...
        # OUTPUT LAYER BEFORE SR:
        self.out_Conv1x1 = Conv_1x1(dim, features_main[0], 1)
        
        # SUPER-RESOLUTION MODULE
        self.SR_Module = SuperResolution_Module(dim, num_channels_SRmodule, conv_kernel_size, activation)

        # OUTPUT LAYER AFTER SR: Add ReLU (enforce non-negativity)
    ...
    def forward(self, x_input):
        ...
        if self.residual_connection:
            ...
            x = y_diff + x_input_oneChannel


        # Super-resolution module with pre-defined layer structure

        x = self.SR_Module(x)
        

        # OUTPUT: Add ReLU (enforce non-negativity)
...
if __name__ == '__main__':
    ...
    random2Dtensor = torch.rand(1, 1, 120, 76)
